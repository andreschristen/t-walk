function [xxp,lt,acc] = twalk_c(logTarget,nsamp,x0,xp0,p)
...
if sum(phi)>0
    sigma = max(abs(xp(phi) - x(phi)))/3;
else
    sigma = 0;
end
h = x + phi.*sigma.*randn(size(x)); % iid standard normal

% evaluate log of ratio of densities g(x|h,x')/g(h|x,x')
lgr = lg3(x,xp,sigma) - lg3(h,xp,sigma);

%-------------------------------------------------------------------------
function lg = lg3(h,xp,sigma)
% log density for hop move, up to constant not affecting ratio

n = length(xp); % number of components in xp
...
% evaluate log of ratio of densities g(x|h,x')/g(h|x,x')
lgr = lg4(x,xp,sigma) - lg4(h,xp,sigma);

%-------------------------------------------------------------------------
function lg = lg4(h,xp,sigma)
