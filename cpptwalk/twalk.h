...
class twalk {
		...
		/*Initial values and the points updated in each iteration*/
	...
	int *select_phi(int *phi)  {
        ...
        public:
        /****Constructor.  krl_probs1  not used, for backward compatibility but it doesn't work! ****/
...
int simulation(int Tr1, const char *filename, char *op="wt") {
...
int simulation(int Tr1, const char *filename, char *op="wt", int save_every1=1, double *xx=NULL, double *xxp=NULL) {
    ...
    if ((fptr = (fopen(filename, op)))) { 
		...
		if (save_every < 0)
			printf("twalk: Only every %d accepted interations will be saved in file %s\n", abs(save_every), filename);
		else
			printf("twalk: Every %d interations to be saved in file %s\n", save_every, filename);
	...
	delete mapx;
		...
		sec = time(NULL);
		printf("twalk: Finished, %4.1f%% of moved pars per it. ratio (%f/%d). Ouput in file %s,\n      %s\n",
